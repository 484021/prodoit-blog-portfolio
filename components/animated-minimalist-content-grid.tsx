...
function ContentSection({ title, items, viewAllHref }: { title: string; items: ContentItem[]; viewAllHref: string }) {
      ...
      <ul className="space-y-2">
        {items.map((item) => (
          <motion.li
            ...
            whileTap={{ scale: 0.95 }}
          >
            <Link href={`/${title.toLowerCase()}/${item.id}`} className="block p-2 rounded-md hover:bg-accent">
              {item.title}
            </Link>
          </motion.li>
        ))}
...
export default function HomeContentTeaser() {
  ...
  return (
    <div className="container mx-auto px-4 py-12">
      ...
      >
        <ContentSection title="Projects" items={projects} viewAllHref="/projects" />
        <ContentSection title="Notes" items={notes} viewAllHref="/notes" />
        <ContentSection title="Blog Posts" items={blogPosts} viewAllHref="/blog" />
      </motion.div>
    </div>
  )
}
